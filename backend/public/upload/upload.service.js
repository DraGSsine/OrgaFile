...
let UploadService = class UploadService {
    ...
    async restoreFile(req, fileId) {
        try {
            ...
            const doc = await this.fileModel.findOneAndUpdate({ userId: req.user.userId }, { $push: { files: restoredFile } }, { upsert: true, new: true });
            return restoredFile;
        }
        catch (error) {
    ...
    async remove(req, fileId, isPermanently) {
        try {
            ...
            if (isPermanently) {
                ...
                const doc = await this.removedFilesModel.findOneAndUpdate({ userId: req.user.userId }, { $push: { files: removedFile } }, { upsert: true, new: true });
                return removedFile;
            }
        }
        catch (error) {
    ...
    async removeMany(req, fileIds, isPermanently) {
        try {
            if (isPermanently) {
                ...
                const doc = await this.removedFilesModel.findOneAndUpdate({ userId: req.user.userId }, { $push: { files: { $each: flattenedRemovedFiles } } }, { upsert: true, new: true });
